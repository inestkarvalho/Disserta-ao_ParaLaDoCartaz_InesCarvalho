...
color[] paleta2 = {#3f22ec, #03e29d}; // laranja, azul 
...
void draw() {
  ...
  if (ligadoAoServidor) {
    dadosRecebidos = client.getSenderData();
    ...
    if (dadosRecebidos.length() > 0) {
      ...
      String[] valores = splitTokens(dadosRecebidos);
...
void camadaveridica() {
  ...
  float yDecide = y5 +(round(width * 0.055)*1.3);
