...
void setup() {
  ...
  client.setReceiverName("CamaraPartilhada");
...
void draw() {
  ...
  //receção dos dados
  boolean ligadoAoServidor = client.receiveTexture();
  String dadosRecebidos = ""; //cria uma variável vazia para os dados

  if (ligadoAoServidor) {
    dadosRecebidos = client.getSenderData(); //Recebe a mensagem enviada pelo servidor

    if (dadosRecebidos != null) {
      dadosRecebidos = trim(dadosRecebidos); //Remove espaços desnecessários
    } else {
      dadosRecebidos = "";
    }

    if (dadosRecebidos.length() > 0) { //Verifica se a mensagem não está vazia

      String[] valores = splitTokens(dadosRecebidos); //separa a mensagem nos quatro valores

      if (valores.length >= 4) {

        try {

          float novaOrigemX = float(valores[0]);
          float novoXbarra = float(valores[1]);
          float novaBarrades = float(valores[2]);
          boolean novoAlvo = int(valores[3]) == 1;


          origemX = novaOrigemX;
          xbarra = novoXbarra;
          barrades = novaBarrades;
          alvo = novoAlvo;
          ...
          println( "CLIENTE | erro ao interpretar: "+ dadosRecebidos);
        }
      } else {

        println("CLIENTE | dados incompletos: "+ dadosRecebidos);
      }
    }
  }


  // Informação na consola para confirmar a ligação
  if (frameCount % 30 == 0) {

    println(
      "CLIENTE | ligado: " + ligadoAoServidor +
      " | emissor: " + client.getSenderName() +
  ...
  if (overlayAtivo) {
    ...
    // Guarda a camada de desinformação numa imagem
    frameCamadaSuperior = get(0, 0, width, height);

    // Desenha por baixo a camada verídica
    background(bg);
    camadaveridica();
  ...
  if (modoDebug) {
    ...
    if (ligadoAoServidor) {
      estadoLigacao = "LIGADO";
    } else {
      estadoLigacao = "SEM LIGAÇÃO";
    }

    text(
      "CLIENTE SPOUT: " + estadoLigacao +
      "\norigemX: " + nf(origemX, 0, 2) +
      "\nxbarra: " + nf(xbarra, 0, 2) +
      "\nbarrades: " + nf(barrades, 0, 2) +
      "\nalvo: " + alvo,
      12,
      12
      );
