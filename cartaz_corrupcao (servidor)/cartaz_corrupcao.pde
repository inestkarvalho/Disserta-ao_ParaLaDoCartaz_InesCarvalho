...
void setup() {
  ...
  if (indiceft==0) {
    ...
    y2 = random(height * 0.17, height * 0.19);
  ...
  boolean emissorCriado = // Cria o emissor e guarda o resultado da operação, ajuda a compreender se criou bem o emissor ou se ha algum erro
    server.createSender("CamaraPartilhada");

  boolean bufferCriado = // buffer guarda origemX xbarra barrades alvo para dar ao cliente
    server.createSenderBuffer(256); // n bytes 

  println("Emissor criado: " + emissorCriado);
  println("Buffer criado: " + bufferCriado);
...
void draw() {
  ...
  if (modoDebug) {
    float camScale = 2;
    int camW = round(cam.width / camScale);
    int camH = round(cam.height / camScale);
    imageMode(CORNER);
    image(cam, 0, 0, camW, camH);     // Força o desenho da câmara à frente do cartaz
    ...
    for (int i = 0; i < faces.length; i++) {
      float fx = map(faces[i].x, 0, cam.width, 0, camW);
      float fy = map(faces[i].y, 0, cam.height, 0, camH);
      float fw = map(faces[i].width, 0, cam.width, 0, camW);
      float fh = map(faces[i].height, 0, cam.height, 0, camH);
  ...
  // Valores que serão enviados para o cliente
  String dadosPartilhados =
    str(origemX) + " " +
    str(xbarra) + " " +
    str(barrades) + " " +
    str(alvo ? 1 : 0);
  ...
  // Envia os valores
  boolean dadosEnviados =
    server.setSenderData(dadosPartilhados);


  // Mostra o estado na consola duas vezes por segundo,
  if (frameCount % 30 == 0) {
    println(
      "SERVIDOR | enviado: " + dadosEnviados +
